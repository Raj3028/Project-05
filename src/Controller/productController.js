...
const getProduct = async (req, res) => {
    ...
    try {
        ...
        if (validator.checkInputsPresent(req.body)) { return res.status(400).send({ status: false, message: "You can't put anything in Body." }) }
        ...
        if (!validator.checkInputsPresent(data)) {
            ...
            if (!validator.checkInputsPresent(productData)) return res.status(404).send({ status: false, message: "Products not found" })
        ...
        if (priceSort || priceSort == '') {
            if (!validator.isValidBody(priceSort)) return res.status(400).send({ status: false, message: "Please enter priceSort!" });
        ...
        if (!validator.checkInputsPresent(getProduct)) return res.status(404).send({ status: false, message: "Product Not Found." })
...
const updateProduct = async (req, res) => {
    ...
    try {
        ...
        let updateProduct = await productModel.findOneAndUpdate({ isDeleted: false, _id: productId }, obj, { new: true })
